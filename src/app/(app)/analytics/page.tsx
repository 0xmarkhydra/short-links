...
export default async function AnalyticsPage() {
  ...
  const [summary, days, devices, browsers] = await Promise.all([
    query<{total:string;top_name:string|null;top_visits:string|null;low_name:string|null;low_visits:string|null}>(
      `WITH owned AS (SELECT id,name,visit_count FROM share_links WHERE user_id=$1 AND status <> 'DELETED')
       SELECT (SELECT COUNT(*)::text FROM visits v JOIN owned o ON o.id=v.link_id) AS total,
  ...
  const s=summary.rows[0]??{total:"0",top_name:null,top_visits:null,low_name:null,low_visits:null};
  ...
  return <><div className="page-heading"><div><span className="eyebrow">ANALYTICS</span><h1>Thống kê truy cập</h1><p>Toàn bộ biểu đồ bên dưới lấy trực tiếp từ database, không sinh số liệu giả.</p></div></div>
...
function Stat({label,value}:{label:string;value:string}){return <div className="stat-card"><span>{label}</span><strong>{value}</strong><small>visit hợp lệ</small></div>}
