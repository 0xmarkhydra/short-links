...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{error?: string; success?: string}> }) {
  ...
  const stats = await query<{total_links: string; total_visits: string; active_links: string; locked_links: string}>(
    `SELECT COUNT(*) FILTER (WHERE status <> 'DELETED')::text AS total_links,
      COALESCE(SUM(visit_count) FILTER (WHERE status <> 'DELETED'),0)::text AS total_visits,
      COUNT(*) FILTER (WHERE status='ACTIVE')::text AS active_links,
      COUNT(*) FILTER (WHERE status='LOCKED')::text AS locked_links
     FROM share_links WHERE user_id=$1`, [user.id]
  );
  ...
  const s = stats.rows[0] ?? {total_links:"0", total_visits:"0", active_links:"0", locked_links:"0"};
  return <>
    ...
    <div className="stats-grid"><Stat label="Tổng Link" value={s.total_links}/><Stat label="Tổng lượt truy cập" value={Number(s.total_visits).toLocaleString("vi-VN")}/><Stat label="Link đang hoạt động" value={s.active_links}/><Stat label="Link bị khóa" value={s.locked_links}/></div>
