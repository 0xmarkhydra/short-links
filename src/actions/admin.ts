...
export async function setUserStatusAction(formData: FormData) {
  ...
  if (!["ACTIVE","LOCKED"].includes(status)) redirect("/admin?error=Trạng+thái+không+hợp+lệ.");
  if (userId === admin.id && status === "LOCKED") redirect("/admin?error=Bạn+không+thể+tự+khóa+tài+khoản+Admin.");
